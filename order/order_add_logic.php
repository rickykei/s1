...
// add new order
if (isset($_POST['importYahoo']) || isset($_POST['importYahooShopping']) || isset($_POST['importRakuten']) || isset($_POST['importRakuten2018'])  || isset($_POST['importAmazon']) ) {
	$import = true;
	$isValid = true;
	...
	if ($isValid) {
		...
		} else if (isset($_POST['importAmazon'])) {
			$import_success = importAmazon($db, $file_name, $actual_file_name, $salesGroup);
		}
	}
...
function importRakuten2018($db, $file_name, $actual_file_name, $salesGroup) {
	...
	//	}
		...
		if (empty($order['sale_ref'])) {
			$error_message = '<br>Line '.$rowNo.': Order No.(column A) is mandatory.';
		//20180531 added 	
		}else if (strlen($order['sale_ref'])>30){
			$error_message = '<br>Line '.$rowNo.': Sale_ref length is more than 30 digits';
		}else if (isOrderExist($db, $order['sale_ref']) || array_key_exists($order['sale_ref'], $orderNoSet)) {
			$error_message = '<br>Line '.$rowNo.': Order No.['.$order['sale_ref'].'] has already existed.';
		}
...
function importAmazon($db, $file_name, $actual_file_name, $salesGroup) {
	global $error_message;
	...
	foreach ($ws->getRowIterator() as $row) {
		...
		$order['sale_yahoo_id'] = '';
		$order['sale_chk_ref'] = 0;
		$order['sale_discount'] = $ws->getCell("AF".$rowNo)->getValue();
		$order['sale_tax'] =  0;

	/** Product */
		$product['sale_ref'] = $order['sale_ref'];
		$product['sprod_name'] = convert($ws->getCell("AB".$rowNo)->getValue());
		$product['sprod_price'] = $ws->getCell("AE".$rowNo)->getValue();
		$product['sprod_unit'] = $ws->getCell("AD".$rowNo)->getValue();
		$product['sprod_id'] = $ws->getCell("AC".$rowNo)->getValue();
	...
	/** Debt */
		...
		$addr = $ws->getCell("R".$rowNo)->getValue();
		...
		$debt['debt_email'] = '';
		$debt['debt_post_co'] = $ws->getCell("W".$rowNo)->getValue();
		$debt['debt_tel'] = $ws->getCell("J".$rowNo)->getValue();
		$debt['debt_mobile'] = '';
		$debt['debt_bank'] = '';
		$debt['debt_pay_method'] = 'Bank in';
		$debt['debt_shipping_method'] = 'Air Mail';
	...
	/** Balance */
		...
		$balance['bal_pay'] = $ws->getCell("AE".$rowNo)->getValue()- $ws->getCell("AF".$rowNo)->getValue();
		...
		//20180807 check productname
		if (strlen($product['sprod_name'])>32){
			$error[$rowNo] .= '<br>Line '.$rowNo.': Product Name length is more than 32 digits';
		} 
		// Validation
		if (empty($order['sale_ref'])) {
			$error_[$rowNo] .= '<br>Line '.$rowNo.': Order No.(column A) is mandatory.';
		//20180531 added 	
		}else if (strlen($order['sale_ref'])>30){
			$error[$rowNo] .= '<br>Line '.$rowNo.': Sale_ref length is more than 30 digits';
		}else if (isOrderExist($db, $order['sale_ref']) || array_key_exists($order['sale_ref'], $orderNoSet)) {
			$error[$rowNo] .= '<br>Line '.$rowNo.': Order No.['.$order['sale_ref'].'] has already existed.';
		}
		 
		
		if (empty($product['sprod_id'])) {
			$error[$rowNo] .= '<br>Line '.$rowNo.': Product No.(column F/I) is mandatory.';
		}
		
		if (empty($product['sprod_name'])) {
			$error[$rowNo] .= '<br>Line '.$rowNo.': Product Name(column E) is mandatory.';
		}
		
		if (empty($product['sprod_price']) && $product['sprod_price'] != '0') {
			$error[$rowNo] .= '<br>Line '.$rowNo.': Product Price(column H) is mandatory.';
		} else if (!is_numeric($product['sprod_price'])) {
			$error[$rowNo] .= '<br>Line '.$rowNo.': Product Price(column H) must be numeric.';
		}
		
		if (empty($product['sprod_unit']) && $product['sprod_unit'] != '0') {
			$error[$rowNo] .= '<br>Line '.$rowNo.': Product Unit(column G) is mandatory.';
		} else if (!is_numeric($product['sprod_unit'])) {
			$error[$rowNo] .= '<br>Line '.$rowNo.': Product Unit(column G) must be numeric.';
		}
		
		if (!empty($error[$rowNo])) {
			continue;
		}
