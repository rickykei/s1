...
function getsale_ref_next()
	...
	$db=connectDatabase();
	mysql_select_db(DB_NAME,$db);
	$result = mysql_query("SELECT max(CONVERT(sale_ref,UNSIGNED INTEGER)) as a FROM ben_sale where sale_chk_ref = 1 order by sale_ref DESC" ,$db) or die (mysql_error()."<br />Couldn't execute query: $query");
